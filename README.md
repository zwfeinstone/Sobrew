...
4. In **Catalog & Pricing**, mark each product Available/Unavailable and set center-specific override price in dollars (stored in `product_prices` center override rows).
...
## Checkout integrity
- Orders are repriced server-side from `product_prices` (center override then tier fallback) inside Postgres RPC `create_order_from_cart`; client cart pricing is ignored.
